...
int parmt_locSearch(const MPI_Comm comm,
                    ...
                    struct parmtData_struct *data, double *phi);
phi = memory_calloc64f(data.nlocs*mtloc.nmtAll);
double *phiLoc = memory_calloc64f(data.nlocs*mtloc.nmtAll);
double mt[6];
...
printf("phiAll\n");
argmax = array_argmax64f(data.nlocs*mtloc.nmtAll, phi);
argmin = array_argmin64f(data.nlocs*mtloc.nmtAll, phi);
idep = argmax/mtloc.nmtAll;
imt = argmax - idep*mtloc.nmtAll;
printf("dep opt; mt opt %d %d\n", idep, imt);
compearth_convertMT(NED, USE, &mtloc.mts[8*imt], mt);
printf("%e,%e,%e,%e,%e,%e\n", mt[0], mt[1], mt[2], mt[3], mt[4], mt[5]);
printf("%d %d %e %e\n", argmin, argmax, phi[argmin], phi[argmax]);
argmax = array_argmax64f(data.nlocs*mtloc.nmtAll, phiLoc);
argmin = array_argmin64f(data.nlocs*mtloc.nmtAll, phiLoc);
idep = argmax/mtloc.nmtAll;
imt = argmax - idep*mtloc.nmtAll;
printf("phiLoc\n");
printf("dep opt; mt opt %d %d\n", idep, imt);
printf("%d %d %e %e\n", argmin, argmax, phiLoc[argmin], phiLoc[argmax]);
compearth_convertMT(NED, USE, &mtloc.mts[8*imt], mt);
printf("%e,%e,%e,%e,%e,%e\n", mt[0], mt[1], mt[2], mt[3], mt[4], mt[5]);
}
...
idep = argmax/mtloc.nmtAll;
imt = argmax - idep*mtloc.nmtAll;
imt = array_argmax64f(nmt, &phi[idep*nmt]);
printf("ln max/min %e %e exp max/min %e %e\n", phi[argmax], phi[argmin], exp(phi[argmax]), exp(phi[argmin]));
xnorm = 0.0;
for (i=0; i<data.nlocs*mtloc.nmtAll; i++){xnorm = xnorm + exp(phi[i]);}
printf("area under joint jpdf %e\n", xnorm);
printf("%d\n", argmax);
printf("%d\n", mtloc.nmtAll);
printf("depopt: %d mtopt %d\n", idep, imt);
printf("best\n");
int ilocOpt = idep;
int imtOpt = imt;
    compearth_convertMT(NED, USE, &mtloc.mts[8*imt], mt);
printf("%e,%e,%e,%e,%e,%e\n", mt[0], mt[1], mt[2], mt[3], mt[4], mt[5]);
idep = argmin/mtloc.nmtAll;
imt = argmin - idep*mtloc.nmtAll;
printf("depbad: %d mtbad %d\n", idep, imt);
compearth_convertMT(NED, USE, &mtloc.mts[8*imt], mt);
printf("%e,%e,%e,%e,%e,%e\n", mt[0], mt[1], mt[2], mt[3], mt[4], mt[5]);

// normalize the pdf
xnorm = array_sum64f(data.nlocs*nmt, phi, &ierr);
cblas_dscal(data.nlocs*nmt, 1/xnorm, phi, 1);
// write the lune
int nloc = data.nlocs;
int nm = mtsearch.nm;
int nb = mtsearch.nb;
                ...
                for (ik=0; ik<nk; ik++)
                {
                    for (is=0; is<ns; is++)
                    {
                        for (it=0; it<nt; it++)
                        {
                            ...
                            int ik_is = is*nk + ik;
                            int ik_it = it*nk + ik;
                            int imt = im*nb*ng*nk*ns*nt
                                    + ib*ng*nk*ns*nt
                                    + ig*nk*ns*nt
                                    + ik*ns*nt
                                    + is*nt
                                    + it; 
                            lune[ig_ib] = lune[ig_ib] + phi[iloc*nmt+imt];
                            if (iloc == ilocOpt){luneOpt[ig_ib] = luneOpt[ig_ib] + phi[iloc*nmt+imt];}
                            strikeSlip[ik_is] = strikeSlip[ik_is] + phi[iloc*nmt+imt];
                            strikeDip[ik_it] = strikeDip[ik_it] + phi[iloc*nmt+imt];
                            dipSlip[it_is] = dipSlip[it_is] + phi[iloc*nmt+imt];
                            loc[iloc] = loc[iloc] + phi[iloc*nmt+imt];
                        }
                    }
                }
            }
