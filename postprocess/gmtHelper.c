    ...
    const int nobs, const struct sacData_struct *data)
{
    ...
    else
    {
        ...
        fprintf(ofl, "gmt psxy $J $R -Sa${ts} -Gyellow -Wblack -O -K << EOF >> ${outps}\n");
        fprintf(ofl, "%8.3f %8.3f\n", globalMap.evlo, globalMap.evla);
        fprintf(ofl, "EOF\n");
    }
                                     ...
                                     const double *__restrict__ thetaHist)
{
    ...
    fprintf(ofl, "psfl=%s\n", psFile);
    fprintf(ofl,
            "gmt psbasemap -JX5i/1i -R0/90/0/%.2f -Bg10a10:\"Dips (deg)\":/a%.2f:\"Likelihood\":WSn -P -K > ${psfl}\n",
            tmax*1.1, tmax*0.2);
    if (nt > 1)
    {
       fprintf(ofl, "gmt psxy -R -J -Wblack -Gred -O -K << EOF >> ${psfl}\n");
    }
    else
    {
       fprintf(ofl, "gmt psxy -R -J -Wblack -Gred -O << EOF >> ${psfl}\n");
    }
    fprintf(ofl, "%f %f\n", 0.0, 0.0);
    fprintf(ofl, "%f %f\n", 0.0, thetaHist[0]);
    for (i=0; i<nt-1; i++)
                                     ...
                                     const double *__restrict__ sigmaHist)
{
    ...
    fprintf(ofl, "psfl=%s\n", psFile);
    fprintf(ofl,
           "gmt psbasemap -JX5i/1i -R-90/90/0/%.2f -Bg15a15:\"Slips (deg)\":/a%.2f:\"Likelihood\":WSn -P -K > ${psfl}\n",
            smax*1.1, smax*0.2);
    if (ns > 1)
    {
       fprintf(ofl, "gmt psxy -R -J -Wblack -Gred -O -K << EOF >> ${psfl}\n");
                                     ...
                                     const double *__restrict__ kappaHist)
{
    ...
    fprintf(ofl, "psfl=%s\n", psFile);
    fprintf(ofl,
            "gmt psbasemap -JX5i/1i -R0/360/0/%.2f -Bg30a30:\"Strike (deg)\":/a%.2f:\"Likelihood\":WSn -P -K > ${psfl}\n",
            kmax*1.1, kmax*0.2);
    if (nk > 1)
    {
       fprintf(ofl, "gmt psxy -R -J -Wblack -Gred -O -K << EOF >> ${psfl}\n");
                                     ...
                                     const double *__restrict__ depHist)
{
    ...
    fprintf(ofl, "psfl=%s\n", psFile);
    fprintf(ofl,
            "gmt psbasemap -JX5i/1i -R%.1f/%.1f/0/%.2f -Bg%fa%f:\"Depths (km)\":/a%.2f:\"Likelihood\":WSn -P -K > ${psfl}\n",
            depMin, depMax, dmax*1.1, dd, (nd-1)*dd/5.0, dmax*0.2);
    if (nd > 1)
    {
       fprintf(ofl, "gmt psxy -R -J -Wblack -Gred -O -K << EOF >> ${psfl}\n");
    }
                                         ...
                                         const double *__restrict__ magHist)
{
    ...
    fprintf(ofl, "psfl=%s\n", psFile);
    fprintf(ofl,
            "gmt psbasemap -JX5i/1i -R%.2f/%.2f/0/%.2f -Bg%fa%f:\"Magnitude (Mw)\":/a%.2f:\"Likelihood\":WSn -P -K > ${psfl}\n",
            mwMin, mwMax, mmax*1.1, dm, (nm-1)*dm/5.0, mmax*0.2);
    if (nm > 1)
    {
       fprintf(ofl, "gmt psxy -R -J -Wblack -Gred -O -K << EOF >> ${psfl}\n");
    }
                                     ...
                                     const double *__restrict__ gammaHist)
{
    ...
    fprintf(ofl, "psfl=%s\n", psFile);
    fprintf(ofl,
            "gmt psbasemap -JX5i/1i -R-30/30/0/%.2f -Bg5a5:\"Longitude (deg)\":/a%.2f:\"Likelihood\":WSn -P -K > ${psfl}\n",
            gmax*1.1, gmax*0.2);
    if (ng > 1)
    {
       fprintf(ofl, "gmt psxy -R -J -Wblack -Gred -O -K << EOF >> ${psfl}\n");
...
//============================================================================//
int postmt_gmtHelper_writeBetaBoxes(const char *outputScript,
                                    const char *psFile,
                                    const int nb,
                                    ...
                                    const double *__restrict__ betaHist)
{
    ...
    fprintf(ofl, "psfl=%s\n", psFile);
    fprintf(ofl,
            "gmt psbasemap -JX5i/1i -R-90/90/0/%.2f -Bg15a15:\"Latitude (deg)\":/a%.2f:\"Likelihood\":WSn -P -K > ${psfl}\n",
            bmax*1.1, bmax*0.2);
    if (nb > 1)
    {
       fprintf(ofl, "gmt psxy -R -J -Wblack -Gred -O -K << EOF >> ${psfl}\n");
